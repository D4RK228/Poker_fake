    ...
    while min != max:
        ...
        for i in range(1, k_player + 1):
            if data_bet[i] <= min:
                ...
                sum += min
                data_bet[i] -= min
            if data_bet[i] == 0 and data_chips[i] == 0 and data_pots[0][i] > 0:
...
def bot_preflop(card1, card2, bet, prev_bet, chips): # bet - разница между последними двумя изменениями ставок 
    x = card1 % 13
    y = card2 % 13
    if x == 0:
        x = 13
...
def bot_flop(card1, card2, data_flop, bet, prev_bet, chips): #если никто не ставил, тогда bet = bigblind
    data_card = []
...
def bot_turn(card1, card2, data_turn, bet, prev_bet, chips):
    data_card = []
    data_card.append(card1)
    data_card.append(card2)
...
def bot_reaver(card1, card2, data_reaver, bet, prev_bet, chips):
    data_card = []
    data_card.append(card1)
    data_card.append(card2)
